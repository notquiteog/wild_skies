...
return function(mod)
  ...
  local function goldNeighborMap(world, id)
    ...
    return {
      id = id, def = def, tileset = tileset, widthCells = w, heightCells = h,
      inBounds = function(_, x, y)
        return x >= 0 and y >= 0 and x < w and y < h
      end,
      isWalkableCell = function(_, x, y)
        return MapDef.defIsWalkableCell ~= nil
          and MapDef.defIsWalkableCell(def, tileset, x, y) == true
      end,
      isWaterCell = function(_, x, y)
        return MapDef.defIsWaterCell ~= nil
          and MapDef.defIsWaterCell(def, tileset, x, y) == true
      end,
