...
local ow = {
  ...
  maps = defs,
  tilesets = { OVERWORLD = { id = "OVERWORLD" } },
...
package.loaded["src.world.Map"] = {
  ...
  defIsWalkableCell = function(def, ts, x, y)
    assert(ts == ow.tilesets.OVERWORLD and type(x)=="number" and type(y)=="number",
      "neighbor ground query must pass native tileset and both coordinates")
    return true
  end,
  defIsWaterCell = function(def, ts, x, y)
    assert(ts == ow.tilesets.OVERWORLD and type(x)=="number" and type(y)=="number",
      "neighbor water query must pass native tileset and both coordinates")
    return false
  end,
}
